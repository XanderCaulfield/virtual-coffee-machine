...
6. Grab the live URL from the top of the service page
   (`https://virtual-coffee-machine.onrender.com` or similar) and smoke-test
   the health check:
...
- **Ephemeral disk.** The SQLite ledger/state database lives on the container's
  filesystem and is **reset on every redeploy or restart** (by design — each
  visitor gets a fresh machine anyway). Don't treat hosted data as durable.
